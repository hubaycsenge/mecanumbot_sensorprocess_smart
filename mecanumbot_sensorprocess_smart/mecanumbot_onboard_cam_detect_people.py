...
# The pose parser library, relative to the directory DeepStream-Yolo-Pose is
# checked out in; `nvinfer_config.BUILD_ROOTS` are the directories searched.
DEEPSTREAM_YOLO_POSE_LIB = os.path.join(
...
class DeepStreamPersonDetectNode(Node):
    ...
    def _custom_lib_path(self, template):
        """
        Return the DeepStream-Yolo-Pose parser library nvinfer should load.
        ...
        found, tried = nvinfer_config.find_custom_lib(
            ...
            DEEPSTREAM_YOLO_POSE_LIB,
        )
        if found:
            self.get_logger().info(f"custom-lib-path: {found}")
            return found
        self.get_logger().error(
            "No DeepStream-Yolo-Pose parser library at any of: " + ", ".join(tried)
            + ". Build it in the DeepStream-Yolo-Pose checkout (make -C "
            "nvdsinfer_custom_impl_Yolo_pose, with CUDA_VER set) or point "
    ...
    def _render_nvinfer_config(self):
        ...
        edited in step by hand, and `custom-lib-path` becomes wherever the parser
        library is on this machine. Without the selected ONNX only the paths are
        rewritten, so the model the template names is the one that runs. Nothing
        else in the file is touched, and the rendered copy is left on disk for
        inspection.
        ...
        try:
            ...
            if onnx is not None:
                substitutions.update({
                    ...
                    "labelfile-path": os.path.join(share, "deepstream_config", "labels.txt"),
                })

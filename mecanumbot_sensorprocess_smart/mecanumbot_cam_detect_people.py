...
class PersonDetectNode(Node):
    def __init__(self,namespace=''):
        ...
        self.get_logger().info(f"Camera source mode: {'topic' if self.from_topic else 'webcam'}")
    ...
    def webcam_callback(self):
        ...
        if not ok:
            self.get_logger().warn(f"Failed to read frame from webcam device: {self.webcam_device}")
            return
